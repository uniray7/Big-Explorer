...
class logs_object:
	...
	def get_MapFeature_list(self):
		feature_list = []
		WordCo_coll = self.connect_DB()
		for job in WordCo_coll.find({"job_name":self.job_name}):
			if job['exec_time'] != -1:
				...
				feature = [MapHeapSize,MapTasksMax,SortMB,SortPer,RecordPer,JVMReuse,job['data_size']]
