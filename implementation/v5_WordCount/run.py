...
def run_job(para_list,num):
	...
	cmd = 'hadoop --config '+EXP+'/conf/conf_new jar '+EXP+'/../hadoop-1*/hadoop-examples* wordcount /text-in'+str(DataSize)+'G /text-out'
	os.system(cmd)

					
	cmd = 'hadoop fs -get /text-out/_logs '+cur_Path()+'/logs/eval_'+str(num)
	os.system(cmd)	


	cmd = 'hadoop fs -rmr /text-out'
	os.system(cmd)
