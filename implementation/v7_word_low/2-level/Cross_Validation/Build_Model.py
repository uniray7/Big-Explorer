...
class WhatIf_Engine:
	...
	def Build_MapDev_Model(self):
		MapDev =  linear_model.LinearRegression()
		MapDev.fit(self.MapFeature_list,self.MapDev_list)
		self.Dump_Model('Model/MapDev.model',MapDev)


	def Build_RedMean_Model(self):
		RedMean = ensemble.RandomForestRegressor(n_estimators=20)
		RedMean.fit(self.RedFeature_list,self.RedMean_list)
		self.Dump_Model('Model/ReduceMean.model',RedMean)
		print RedMean.feature_importances_

	def Build_RedDev_Model(self):
		RedDev =  linear_model.LinearRegression()
		RedDev.fit(self.RedFeature_list,self.RedDev_list)
		self.Dump_Model('Model/ReduceDev.model',RedDev)
	...
	def Build_Final_Model(self):
		...
		Final.fit(interData_list,self.Target_list)
		self.Dump_Model('Model/Job.model',Final)
